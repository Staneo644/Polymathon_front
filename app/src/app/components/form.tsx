...
export default function form(content:string[]): JSX.Element {
    ...
    const [searchWord, setSearchWord] = useState(content.length > 0 ? content[0] : '');
    const [definition, setDefinition] = useState(content.length > 2 ? content[1] : '');
    const [etymology, setetymology] = useState(content.length > 1 ? content[2] : '');
    ...
    return (
      <div className="flex flex-col space-y-4 items-center h-100 w-100 justify-evenly text-black">
        ...
        ></textarea>
        <button className="bg-blue-500 text-white py-2 px-4 rounded max-w-[700px]" onClick={() => {setOnSubmit(true); addWord()}}>
          Soumettre
        </button>
      </div>
    );
